...
class Renderer {
    ...
    adjustSize() {
        ...
        // CSS locks .game-container to 9:16 via explicit min() width+height rules,
        // so scale = contW/1080 always keeps every element inside the container.
        //
        // clientHeight is intentionally NOT used for scale: on Android WebView it
        // can return 0 on the first layout pass (timing bug), which would zero-out
        // every CSS variable.  clientWidth is reliably non-zero from the start.
        //
        // Edge-to-edge (nav bar clipping) is fixed at the Android Java level in
        // MainActivity.java by calling WindowCompat.setDecorFitsSystemWindows(true)
        // after Capacitor's super calls, ensuring 100vh/100svh equal the visible
        // area rather than the full physical screen height.

        const gameContainer = document.querySelector('.game-container');
        const contW = gameContainer.clientWidth;
        if (!contW) return;   // guard: abnormal 0 value, wait for resize event
        ...
        //
        // Heights are omitted: CSS uses aspect-ratio (1060/317 and 980/338) so
        // the browser computes height from width, preventing floating-point
        // rounding distortion from independent JS width/height calculations.

        // Width 먼저 정수 반올림 → height는 그 width에서 역산
        // 이렇게 하면 background-size:100% 100% 에서 왜곡이 발생하지 않는다.
        // (width, height를 독립적으로 계산하면 각각 반올림 오차가 달라 비율이 틀어짐)
        const headerW = Math.round(1060 * scale);
        const headerH = Math.round(headerW * 317 / 1060);
        css('--header-left',     10 * scale);
        css('--header-top',      40 * scale);
        css('--header-w',        headerW);
        css('--header-h',        headerH);
        css('--header-num-size', 100 * scale);

        const guideW = Math.round(980 * scale);
        const guideH = Math.round(guideW * 338 / 980);
        css('--guide-left',      50 * scale);
        // bottom 기준 포지셔닝: 컨테이너 하단에서 22 ref units 위
        // top 기준이면 컨테이너 높이 계산 오차가 잘림으로 직결되지만,
        // bottom 기준이면 컨테이너가 어떻게 계산되든 guide bar가 항상 보임.
        css('--guide-bottom',    22 * scale);
        css('--guide-w',         guideW);
        css('--guide-h',         guideH);
        css('--guide-font-size', 40 * scale);
